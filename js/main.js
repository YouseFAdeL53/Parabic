...
window.onscroll = function () {
  if (window.scrollY >= mySkillsSection.offsetTop - 100) {
...
myImages.forEach((img) => {
  img.addEventListener("click", (e) => {
    ...
    let contentParagph = document.createTextNode(
      "نحن نفهم احتياجاتك ونعمل على تحقيق أهدافك"
    );
...
workImage.forEach((photo) => {
  photo.addEventListener("click", () => {
    ...
    let paraghText = document.createTextNode(
      "نحن نفهم احتياجاتك ونعمل على تحقيق أهدافك"
    );
